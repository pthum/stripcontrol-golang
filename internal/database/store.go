...
type DBReader interface {
	...
	Get(id string, obj any) error
	Close()
}

type DBWriter interface {
	Save(input any) (err error)
	Update(dbObject any, input any) (err error)
	Create(input any) (err error)
	Delete(input any) (err error)
	Close()
}
...
func (d *GeneralDbHandler) GetAll(targets any) (err error) {
	return d.db.Find(targets).Error
}

// Get loads an object from the database
func (d *GeneralDbHandler) Get(ID string, target any) (err error) {
	return d.db.Where("id = ?", ID).First(&target).Error
}

// Update updates the object
func (d *GeneralDbHandler) Update(dbObject any, input any) (err error) {
...
func (d *GeneralDbHandler) Create(input any) (err error) {
	return d.db.Create(input).Error
}

func (d *GeneralDbHandler) Save(input any) (err error) {
	return d.db.Debug().Save(input).Error
}

func (d *GeneralDbHandler) Delete(input any) (err error) {
	return d.db.Delete(input).Error
}

// findPartialUpdateFields find the fields that need to be updated
func findPartialUpdateFields(dbObject any, input any) (fields []string) {
